...
#getwd()
#contoh: ---- setwd("C:/Users/unknown/Downloads/R_presentasi/R-tidyverse")
#dir()

#import gapminder data from "library gapminder"
gapminder <- gapminder
...
#wardata: subset only 10 rows, and first 20 columns --> subset[]
dt_1 <- wardata[1:10, 1:20]

#wardata: subset only country and year, in 10 first rows --> subset[, c()]
...
#mutate : changing values in pop variable (with a million) # sort desc order
...
gapminder_gdp
#shorten gdp values into million USD #filter only 2007
...
#gapminder_gdp: summary mean_gdp = mean(gdp), group_by continent
gapminder_avg_gdp <- 
...
# We will use the R built-in iris data set, which we start by converting into a tibble data frame (tbl_df) for easier data analysis.
...
#keep only non-duplicate rows (!duplicated) based on a specific variable (Sepal.Length)
unique_Sepal <- spesies_data[!duplicated(spesies_data$Sepal.Length),]
  
#check dimension unique_Sepal, to see how many rows are left
dim(unique_Sepal)
...
#remove duplicated rows using distinct()-  based on all columns:
...
#exppop: create similar unique identifier to wardata -- idyear variable
...
#exppop: has similar variable name with wardata ---> "pop" variable name must be changed to "pop_ex"
...
#dataset: look closer to the merged variable 
